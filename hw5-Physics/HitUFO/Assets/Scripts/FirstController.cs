...
{
    private UserGUI gui;
    private TotalActionManager actionManager;
    ...
    void Awake()
    {
        ...
        actionManager = gameObject.AddComponent<TotalActionManager>() as TotalActionManager;
    ...
    {
        InvokeRepeating("SendDisk", 1, 1.5f);//3s per disk
    ...
    public void SendDisk()
    {
        ...
        actionManager.ThrowUFO(diskFactory.GetDisk(round));
    }  
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Fire1"))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                if(ufoHitted.GetComponent<Renderer>().material.color == Color.black && ufoHitted.name!="Ground")
                {
                    score++;
                    diskFactory.FreeDisk(ufoHitted.GetComponent<UFOScript>().manager);
                    ufoHitted.SetActive(false);
                }
                else if(ufoHitted.name != "Ground")
                {
                    score += 2;
                    diskFactory.FreeDisk(ufoHitted.GetComponent<UFOScript>().manager);
                    ufoHitted.SetActive(false);
